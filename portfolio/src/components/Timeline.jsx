...
// Badge for one timeline entry — shows the entry's logo image if provided
// and loadable, otherwise falls back to the type icon (education/
// experience/milestone) so there's never a broken image.
function EntryBadge({ entry }) {
  const [failed, setFailed] = useState(false)
  const Icon = ICONS[entry.type] ?? FiFlag
  const showImage = Boolean(entry.image) && !failed

  return (
    <div
      className={`relative z-10 flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-2xl border glass ${COLORS[entry.type]}`}
    >
      {showImage ? (
        <img
          src={entry.image}
